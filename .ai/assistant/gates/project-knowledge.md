...
Canonical owner: `ALATYR-KNOWLEDGE-001`.

Before a material task ends, determine whether captured conclusions contain a
...
Before task-time delivery, require a profile plus at least one stronger area,
subsystem, architecture item, dependency, fact, contract, path, symbol, or
issue signal. Reverify the
canonical owner. Only `accepted` plus `current` may be supplied as a current
constraint. Treat stale items as warnings and contradictions as blockers.
